...
const VerifyOtp = ({ route }) => {
  ...
  const { resetCode, loading, error } = useSelector(state => state.forgetData);
  ...
  const handleVerifyOtp = async () => {
    try {
      ...
      if (!newPassword || newPassword.trim() === '') {
        dispatch(
          showMessage({ type: 'error', text: 'Please enter new password.' }),
        );
        return;
      }
  ...
  return (
    <SafeAreaView style={styles.pageBg} edges={['top']}>
      ...
      <AppText style={styles.headerTextStyle}>Verify OTP</AppText>
      <AppText style={styles.descriptionTextStyle}>
        Enter the OTP sent to your email and set a new password
      </AppText>
      <View style={styles.inputGroup}>
        <AppText style={styles.label}>Your Otp</AppText>
        <TextInput
          style={styles.input}
      ...
      <View style={styles.buttonsRowStyle}>
        {/* <TouchableOpacity style={[styles.buttonStyle, { width: width * 0.4, borderWidth: 1, borderColor: '#0000', borderRadius: 10, }]}>
                    <AppText style={styles.buttonTextStyle}>Cancel</AppText>
                </TouchableOpacity> */}
        <TouchableOpacity
          style={[
            styles.buttonStyle,
            { width: width * 0.4, backgroundColor: '#E5B865' },
          ]}
        ...
        >
          {loading ? (
            ...
            <AppText style={[styles.buttonTextStyle, { color: '#fff' }]}>
              Reset Password
            </AppText>
          )}
        </TouchableOpacity>
