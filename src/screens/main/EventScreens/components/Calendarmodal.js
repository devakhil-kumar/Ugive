...
const CalendarModal = ({ visible, onClose }) => {
  ...
  useEffect(() => {
    if (selectedDate) {
      // Agar pehle se hi kisi alag date ka panel khula tha, toh height jump na ho
      // iske liye pehle collapse karo (no animation) fir smoothly expand karo
      if (
  ...
  // ── Animated loader fade (no mount/unmount -> no layout jump) ─────────────
  const loaderAnim = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    Animated.timing(loaderAnim, {
      toValue: calLoading ? 1 : 0,
      duration: 180,
    ...
    >
      <View style={styles.modalOverlay}>
        ...
        <View style={styles.modalSheet}>
          ...
          {/* Loading indicator — always mounted (fixed height), sirf opacity fade hota hai
              taaki neeche wala event panel achanak upar-neeche shift na ho (no flicker) */}
          <Animated.View
            style={[styles.calLoader, { opacity: loaderAnim }]}
            pointerEvents="none"
          >
            <ActivityIndicator size="small" color="#F3B11C" />
            <AppText style={styles.calLoaderText}>Loading events…</AppText>
