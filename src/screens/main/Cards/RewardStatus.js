...
const RewardStatus = () => {
  ...
  const { currentReward, loading, error } = useSelector(
    ...
    return (
      <GradientScreen colors={['#6D5B98']}>
        ...
        <View style={styles.errorContainer}>
          <AppText style={styles.errorText}>Error loading rewards</AppText>
          <TouchableOpacity
    ...
    <GradientScreen colors={['#6D5B98']}>
      ...
      <View style={styles.pageBg}>
        <AppText style={styles.header}>
          Send {remainingCards} more {remainingCards === 1 ? 'card' : 'cards'}{' '}
          to receive a free{' '}
          <AppText style={{ color: '#D99656' }}>{rewardName}.</AppText>
        </AppText>
        <ProgressPieChart percentage={percentage} />
        <TouchableOpacity style={styles.btnLets} onPress={handleModal}>
          <AppText style={styles.txtBtn}>Start Writing</AppText>
        </TouchableOpacity>
      </View>
    </GradientScreen>
  );
